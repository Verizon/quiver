...
case class GrContext[A,B](inEdges: IntMap[Vector[B]], label: A, outEdges: IntMap[Vector[B]])
...
object Graph {
  ...
  def addSucc[A,B](g: GraphRep[A, B], v: Node, lps: Vector[(B, Node)]): GraphRep[A,B] =
    if (lps.isEmpty) g else addSucc(adjust(g, lps.head._2, { (x: GrContext[A,B]) => x match {
      case GrContext(ps, lp, ss) =>
        GrContext(ps, lp, ss.updateWith(v, Vector(lps.head._1), _ ++ _))
    }}), v, lps.tail)
  ...
  def addPred[A,B](g: GraphRep[A, B], v: Node, lss: Vector[(B, Node)]): GraphRep[A,B] =
    if (lss.isEmpty) g else addPred(adjust(g, lss.head._2, { (x: GrContext[A,B]) => x match {
      case GrContext(ps, lp, ss) =>
        GrContext(ps.updateWith(v, Vector(lss.head._1), _ ++ _), lp, ss)
    }}), v, lss.tail)
  ...
  */
  ...
  def addEdge(e: LEdge[B]): Graph[A,B] = {
    ...
    def addSuccP(p: GrContext[A,B]) = {
      ...
      GrContext(ps, lp, ss.updateWith(w, Vector(l), _ ++ _))
    }
    def addPredP(p: GrContext[A,B]) = {
      val GrContext(ps, lp, ss) = p
      GrContext(ps.updateWith(v, Vector(l), _ ++ _), lp, ss)
    }
    val g1 = adjust(rep, v, addSuccP)
    Graph(adjust(g1, w, addPredP))
  }
  ...
  def removeLEdge(e: LEdge[B])(implicit eq: Equal[B]): Graph[A,B] = decomp(e.from) match {
    case Decomp(None, _) => this
    case Decomp(Some(Context(p, v, l, s)), gp) =>
  ...
  def gmap[C,D](f: Context[A,B] => Context[C,D]): Graph[C,D] =
    fold(Graph.empty[C,D]) { (c, g) => g & f(c) }
